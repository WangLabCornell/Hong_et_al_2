...
for n = 1:num_of_traces
    ...
    DAQ_single = result_C_eff_vs_force_A43_C109_b6_0200;
    
    force_single_LUT = DAQ_single(:,1);
    Ceff_single_LUT = DAQ_single(:,2);
    Force = 3;%pN,%-----------------------------------------------------
    Ceff_single = interp1(force_single_LUT,Ceff_single_LUT,Force);
    buckling_torque = 12.7825 * Force.^0.632447;
    sigma_single_s = buckling_torque/Ceff_single/kBT/w0;
    count_single_DNA = 1;
    sigma_single_ROI = 0:0.001:0.15;% upper limit should be smaller than sigma_p--------------------------------
    for sigma_single = sigma_single_ROI
        if sigma_single < sigma_single_s
            torque_single(count_single_DNA) = Ceff_single*kBT*w0*sigma_single;
        elseif sigma_single >= sigma_single_s
            torque_single(count_single_DNA) = buckling_torque;
        end
        count_single_DNA = count_single_DNA + 1;
    end
    figure(6)
    plot(sigma_single_ROI,torque_single)
    hold on
    
    a_torque_comparison = {sigma_single_ROI' torque_single' sigma_braid' torque_average_unshifted'};
