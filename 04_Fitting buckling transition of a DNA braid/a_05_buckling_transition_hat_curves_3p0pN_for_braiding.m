...
for n = 1:num_of_traces
    ...
    turn_current_pos_ROI = turn_current(turn_current > 3);
    ext_current_pos_ROI = ext_current(turn_current > 3);
    %[fitresult_pos_Ca_hat_curve, gof_pos_Ca_hat_curve] = pos_Ca_hat_curve_Fit(turn_current_pos_ROI, ext_current_pos_ROI);
    [fitresult_pos_Ca_hat_curve, gof_pos_Ca_hat_curve] = buckling_transition_Fit_v2(turn_current_pos_ROI, ext_current_pos_ROI);
    pos_transition_Ca_from_HC(n) = fitresult_pos_Ca_hat_curve.xb;

    
end
a_characteriza_hat_curve = [mean(pos_transition_Ca_from_HC) std(pos_transition_Ca_from_HC)]

%convert to signa
Lk0 = 7092/10.5;
pos_transition_sigma_from_HC = pos_transition_Ca_from_HC/Lk0;

a_characteriza_hat_curve_sigma = [mean(pos_transition_sigma_from_HC) std(pos_transition_sigma_from_HC)]